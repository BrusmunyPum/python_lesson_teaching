...
def show_header() -> None:
    ...
    subtitle = Text("Rich-powered CRUD app • colorful • professional • easy to use", style="bright_cyan")
    console.print(
        Panel(
...
def build_dashboard(products: List[Product]) -> Panel:
    ...
    cards = [
        ...
        Panel(f"[bold white]{', '.join(categories) if categories else '-'}[/bold white]\nCategories", border_style="bright_blue", box=box.ROUNDED),
    ]
    return Panel(Columns(cards, expand=True), title="[bold bright_cyan]Dashboard[/bold bright_cyan]", border_style="bright_blue")


def render_products_table(products: List[Product], title: str = "Products", page: int = 1, page_size: int = 5) -> None:
    if not products:
        console.print(Panel("[yellow]No products found.[/yellow]", border_style="yellow"))
        return

    total_pages = max(1, ceil(len(products) / page_size))
    page = max(1, min(page, total_pages))
    start = (page - 1) * page_size
    end = start + page_size
    visible_rows = products[start:end]

    table = Table(
        title=f"{title} [dim](Page {page}/{total_pages})[/dim]",
        box=box.HEAVY_HEAD,
        header_style="bold white on dark_blue",
        row_styles=["none", "dim"],
        expand=True,
    )
    table.add_column("ID", style="cyan", justify="center", width=8)
    table.add_column("Product Name", style="bold white", min_width=20)
    table.add_column("Category", style="magenta", min_width=16)
    table.add_column("Qty", justify="right", style="green", width=8)
    table.add_column("Price", justify="right", style="yellow", width=12)
    table.add_column("Value", justify="right", style="bright_cyan", width=14)
    ...
    if total_pages > 1:
        console.print("[dim]Use N for next page, P for previous page, or B to go back.[/dim]")


def paginated_view(products: List[Product], title: str) -> None:
    if not products:
        render_products_table(products, title)
        pause()
        return
    ...
    while True:
        console.clear()
        show_header()
        console.print(build_dashboard(load_products()))
        render_products_table(products, title, page=page, page_size=page_size)
        ...
        action = Prompt.ask("[bold white]Action[/bold white]", choices=["n", "p", "b"], default="b").lower()
...
def update_product() -> None:
    ...
    while True:
        render_products_table([product], "Selected Product", page_size=1)
        console.print(
            Panel(
...
def delete_product() -> None:
    ...
    render_products_table([product], "Product to Delete", page_size=1)
...
def search_products() -> None:
    while True:
        ...
        if choice == "1":
            ...
            product = find_product_by_id(products, pid)
            paginated_view([product] if product else [], f"Search Result for ID: {pid}")
        elif choice == "2":
            keyword = Prompt.ask("Enter product name").lower().strip()
            matches = [p for p in products if keyword in p["pname"].lower()]
            paginated_view(matches, f"Search Result for Name: {keyword}")
        elif choice == "3":
            keyword = Prompt.ask("Enter category").lower().strip()
            matches = [p for p in products if keyword in p["pcat"].lower()]
            paginated_view(matches, f"Search Result for Category: {keyword}")
        elif choice == "4":
            price = FloatPrompt.ask("Enter product price")
            matches = [p for p in products if p["price"] == price]
            paginated_view(matches, f"Search Result for Price: ${price:,.2f}")
        else:
            break
